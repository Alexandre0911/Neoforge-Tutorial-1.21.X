package net.alex.tutorialmod.block.custom;
...
public class MagicBlock extends Block {
    ...
    public void stepOn(Level level, BlockPos pos, BlockState state, Entity entity) {
        ...
        if (entity instanceof ItemEntity itemEntity) {
            // Check if item is bismuth. If it is, transform it into a diamond
            if (itemEntity.getItem().getItem() == ModItems.BISMUTH.get()) {
                itemEntity.setItem(new ItemStack(Items.DIAMOND, itemEntity.getItem().getCount()));
            }

            // Check if item is a dandelion. If it is, transform it into a poppy
            if (itemEntity.getItem().getItem() == Items.DANDELION) {
                itemEntity.setItem(new ItemStack(Items.POPPY, itemEntity.getItem().getCount()));
            }
        }
